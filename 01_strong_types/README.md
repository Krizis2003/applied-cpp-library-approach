# Примеры к статье «C++ и шаблоны: на шаг дальше прикладного минимума. Сильные типы»

Этот репозиторий содержит компактный набор примеров, сопровождающих статью о сильных типах, шаблонных обёртках, диспетчеризации по типам и других простых приёмах, которые позволяют аккуратно выйти за пределы «шаблонного минимализма» в прикладном C++.
...
Шаблонная обёртка `TypeWrapper<T, Tag>`: универсальный каркас и безопасная индексация (Strong ID).
...
Современные возможности: Designated Initializers и User-Defined Literals (`_m`) для лаконичности кода.

**06.** `06_parameter_object.cpp`  
Паттерн Parameter Object: прагматичный компромисс для функций с большим количеством параметров.
