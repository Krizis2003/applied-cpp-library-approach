...
Технически, наш `TypeWrapper` можно научить автоматически генерировать нужные операторы. Однако методы для такой автоматизации (Mixins, CRTP) мы еще не рассматривали. К тому же, разработка и поддержка собственного решения часто обходятся дороже, чем освоение готового инструмента. В таких случаях рациональнее не изобретать велосипед, а сразу обратиться к возможностям профессиональных библиотек.

<a id="libraries"></a>
### Уровень 2: Готовые библиотеки

Что делать, если типов становятся десятки? Прописывать вручную арифметику, сравнения и операторы вывода для каждой новой сущности — это путь к опечаткам и раздуванию кодовой базы. Здесь на смену ручному проектированию приходят готовые решения (вроде [NamedType](https://github.com/joboccara/NamedType), [strong_type](https://github.com/rollbear/strong_type) или [type_safe](https://github.com/foonathan/type_safe)). 
...
Насколько эти техники актуальны в эпоху «современного» C++? Под этим термином мы объединяем возможности, появившиеся начиная с переломного стандарта C++11. Разберем, как встроенные инструменты языка взаимодействуют с нашими обертками и где они могут их заменять.
