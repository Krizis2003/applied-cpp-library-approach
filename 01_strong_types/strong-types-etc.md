...
- [C++: Техники за пределами прикладного минимума. Сильные типы](#strong-types)
  ...
  - [Возможности и области применения](#scaling-up)
    - [1. Защита логики взаимодействия](#logic-protection)
    - [2. Возможность перегрузки функций](#overloading-bonus)
    - [3. Гибкий порядок параметров](#flexible-order)
  - [Не всё так просто: немного о практике](#rationality)
  - [Инструменты оптимизации: как платить меньше](#optimization)
    - [Уровень 1: Шаблоны (TypeWrapper)](#templates-simplification)
    - [Уровень 2: Готовые библиотеки](#libraries)
    - [Прагматичный подход: когда стоит остановиться?](#pragmatic-balance)
...
> **Важное уточнение:** Может возникнуть иллюзия, что «сильные типы» — это какая-то альтернатива именно примитивным типам (вроде `double` или `int`). На самом деле, речь идет о **различимости** типов для компилятора. Если ваша функция принимает аргументами три одинаковые структуры — возникает та же проблема, что и с `double`. Сильные типы позволяют сделать разные по смыслу сущности разными для системы типов, даже если их внутреннее устройство идентично.
...
<details>
  ...
  Под «нулевой стоимостью» подразумевается идентичность машинного кода для `double` и `Meters` при включенных оптимизациях (`-O2`). Чтобы магия работала, тип должен оставаться **тривиальным**:
...
Однако в реальных проектах чаще приходится иметь дело с рефакторингом. У вас уже есть сотни функций и тысячи строк кода, которые нужно адаптировать. Именно здесь возникает самый сильный соблазн — сделать сильные типы «прозрачными», добавив оператор неявного приведения к базовому типу. Это кажется быстрым способом «просто заставить всё работать», но на практике такая уступка часто сводит на нет всю защиту.
...
Чтобы гарантировать защиту, необходим `explicit`. Это наша своего рода **цена за безопасность**: вместе с ним приходится прописывать и операторы для защиты логики, о которых говорилось выше. В сумме это всегда дополнительный код, и в масштабах проекта его объем может быть внушительным. Именно в этой точке приходится решать: окупает ли контроль типов затраты на написание сопутствующего кода?
...
В привычном прикладном программировании тип часто воспринимается как «чертёж» объекта: у него есть поля, методы или хотя бы размер. Даже при использовании типов в шаблонах от них ожидается наличие информации. В `std::vector<int>` тип `int` говорит вектору, сколько памяти выделить и как копировать элементы. В `std::map<Key, Value>` типы определяют, как сравнивать ключи.

Здесь же используется **пустая структура**, в которой нет ничего. Она не несёт данных, не занимает места и никак не влияет на работу процессора. 

Это принципиальный момент: тип используется не ради его содержимого, а исключительно ради его **идентичности**. Для компилятора `TypeWrapper<double, MetersTag>` и `TypeWrapper<double, SecondsTag>` — это две абсолютно разные, несовместимые вселенные, просто потому что у них разные «паспорта»-теги.

Такое использование типа как «маркера» или «имени» — одна из ключевых идей, которые лежат за границей привычного минимума. Это переход от программирования *данными* к программированию *информацией о типах*. Именно эта «пустота» тега позволяет масштабировать систему, создавая новые сущности простым объявлением имени.
...
Помимо автоматизации инфраструктуры, шаблонный подход дает удобный бонус — возможность «подключить» все наши типы к стандартной библиотеке одним блоком кода. Например, благодаря единому шаблону `TypeWrapper` проблему можно решить один раз для всех производных типов, просто специализировав `std::hash`:
...
Этот подход решает проблему создания типов, но оставляет нас в полупозиции. Наш `TypeWrapper` взял на себя инфраструктурный код (конструкторы, хеши), но доменные правила (арифметика, сравнения) по-прежнему требуют ручного труда. 
...
Такая автоматизация смещает тот самый **баланс усилий**, о котором говорилось ранее, в пользу разработчика. Библиотеки позволяют получить преимущества **сильных типов**, не раздувая кодовую базу. Это делает инструмент оправданным в гораздо большем числе сценариев, но окончательное решение о целесообразности его применения в конкретной точке по-прежнему остается за инженером.
...
*   **Ограничения «гибкого порядка»:** Не стоит внедрять сильные типы ради того, чтобы сделать порядок аргументов неважным, если их больше двух. Как показано ранее, количество перегрузок в этом случае растет факториально, и «магия» удобства быстро превращается в нагромождение дублирующего кода.
...
А здесь современные возможности не конкурируют с приёмом, а становятся для него логичным дополнением.
...
3.  **Пустой тип** (маркер смысла) — практически не имеет накладных расходов в рантайме (подробнее об их оптимизации речь пойдет в следующей части).

Разбор такой «анатомии» помогает увидеть за сложным приёмом взаимодействие простых деталей и перестать воспринимать подобные техники как неприступный монолит. Из этого списка лишь пустые типы редко встречаются в прикладном коде, так и должно быть, — в дальнейшем будет показано, что и другие решения строятся из подобных элементов, а их основа во многом повторяется.
